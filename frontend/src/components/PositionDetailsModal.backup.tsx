          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <Paper sx={{ p: 2 }}>
                ...
                <List>
                  {positionDetails?.events.map((event, index) => (
                      ...
                      <ListItemText
                        primary={
                          <Typography variant="subtitle2">
                            {event.event_type.toUpperCase()} {Math.abs(event.shares)} shares @ {formatCurrency(event.price)}
                          </Typography>
                        }
                        secondary={
                          <Box>
                            ...
                            {event.realized_pnl && (
                              ...
                              >
                                P&L: {formatCurrency(event.realized_pnl)}
                              </Typography>
                            )}
                            {event.notes && (
              ...
              <Paper sx={{ p: 2 }}>
                ...
                <TableContainer sx={{ maxHeight: 400 }}>
                  <Table stickyHeader size="small">
                    ...
                    <TableBody>
                      {positionDetails?.events.map((event) => (
                        <TableRow key={event.id}>
                          ...
                          <TableCell align="right">
                            {event.event_type === 'buy' ? '+' : '-'}{Math.abs(event.shares)}
                          </TableCell>
                          <TableCell align="right">
                            {formatCurrency(event.price)}
                          </TableCell>
                          <TableCell align="right">
                            {formatCurrency(Math.abs(event.shares) * event.price)}
                          </TableCell>
                          <TableCell align="right">
                            {event.realized_pnl ? (
                              <Typography 
